(() => {
    ...
    (() => {
        ...
        function menuInit() {
            ...
            if (document.querySelector(".icon-menu-foot")) document.addEventListener("click", (e => {
                if (bodyLockStatus && e.target.closest(".icon-menu-foot")) {
                    bodyLockToggle();
                    document.documentElement.classList.toggle("menu-open");
                }
            }));
        ...
        function initSliders() {
            if (document.querySelector(".experts__slider")) new swiper_core_Swiper(".experts__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3.5,
        ...
        function big_window() {
            ...
            items.addEventListener("click", (e => {
                if (e.target.closest(".card")) {
                    let key = e.target.closest(".card").dataset.column;
                    items.setAttribute("data-box-nov", `${key}`);
                }
            }));
        }
        big_window();
        window.addEventListener("resize", (e => {
            let sizeWind = window.innerWidth;
            textAreaChange(sizeWind);
        }));
        function textAreaChange(s) {
            let textArr = document.querySelector(".comments__text");
            if (s <= 479) textArr.setAttribute("rows", "5"); else textArr.setAttribute("rows", "10");
        }
        window["FLS"] = true;
        isWebp();
        addTouchClass();
